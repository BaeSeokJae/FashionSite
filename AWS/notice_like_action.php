...
$id = $_GET[id];
$number = $_GET[number];
$board_like = "select * from notice_like WHERE id = '$id' AND like_id = '$number'";
$result = $connect->query($board_like);
$like = mysqli_fetch_assoc($result);
$board = "select * from notice where number ='$number'";
...
<?php } else if ($like['id'] == $id && $like['like_id'] == $number) {
    $delete = "DELETE FROM notice_like WHERE id = " . "'$id' AND like_id = '$number'";
    $result1 = $connect->query($delete);
    $hit = "update notice_like set like_num = like_num -1";
    $result = $connect->query($hit);
    $hit1 = "update notice set content_like = content_like -1 where number = '$number'";
    $result = $connect->query($hit1);
    $reset = "ALTER TABLE notice_like AUTO_INCREMENT = 1";
...
<?php } else if ($_SESSION['userid'] != null) {
    ...
    $query = "insert into notice_like (number, like_id, like_num, status, id)
